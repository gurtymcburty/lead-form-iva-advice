...
async function scrapeTypeform(url: string, formName: string): Promise<FormData> {
  ...
  try {
    ...
    const styles = await page.evaluate(() => {
      ...
      // Try to find main elements and get their styles
      const buttons = document.querySelectorAll('button');
      const inputs = document.querySelectorAll('input');
      const texts = document.querySelectorAll('h1, h2, p, span, div');
    ...
    while (stepNumber < maxSteps && consecutiveNoProgress < maxNoProgress) {
      ...
      if (stepInfo.options.length > 0) {
        console.log(`   Options: ${stepInfo.options.join(', ')}`);
      }
...
async function tryAdvanceForm(page: Page, stepInfo: { fieldType: string; options: string[] }): Promise<boolean> {
  try {
    // Handle different field types
    if (stepInfo.fieldType === 'multiple_choice' && stepInfo.options.length > 0) {
      // Click first option for multiple choice
    ...
    const buttonClicked = await page.evaluate(() => {
      ...
      // Try all visible buttons
      const buttons = document.querySelectorAll('button');
      for (const btn of buttons) {
        if ((btn as HTMLElement).offsetParent !== null) {
          const text = btn.textContent?.toLowerCase() || '';
          if (text.includes('ok') || text.includes('next') || text.includes('continue') || text.includes('submit')) {
            btn.click();
            return true;
          }
        }
...
async function main() {
  ...
  for (let i = 0; i < minSteps; i++) {
    if (form1.steps[i].questionText !== form2.steps[i].questionText) {
      comparison.differences.push(`Step ${i + 1} differs: "${form1.steps[i].questionText.substring(0, 50)}" vs "${form2.steps[i].questionText.substring(0, 50)}"`);
    }
  }
